...
const uint32_t tWhite[] = {0x00070707, 0x001F1F1F, 0x003F3F3F, 0x005F5F5F, 0x007F7F7F};
...
void Ledbar::startBle(uint16_t width, uint16_t height)
{
  _width = width;
  _height = height;
  _bleIndex = 0;
...
void Ledbar::putBle(const uint8_t* buff, uint8_t sz)
{
  uint8_t* pImage = &_share._image[0][0];
  
  for(int i = 0 ; i < sz ; i++)
  {
    uint8_t val = *(buff + i);
    uint8_t len = (val & 0x0F) + 1;
    uint8_t col = val >> 4;

    for(int j = 0 ; j < len ; j++)
    {
      if(_bleIndex & 0x0001){   // 奇数
        *(pImage + (_bleIndex >> 1)) |= col;
      }else{                    // 偶数
        *(pImage + (_bleIndex >> 1)) = col << 4;
      }
      _bleIndex++;
    }
  }
...
#define VIEW_WIDTH      (48)    // 表示幅
#define SCROLL_OFFSET   (8)    // スクロール幅

uint16_t width_index = 0;
